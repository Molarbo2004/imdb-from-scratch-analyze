...
# GRIDSEARCH ДЛЯ NAIVE BAYES
from sklearn.model_selection import GridSearchCV

print("Поиск оптимальных параметров для Naive Bayes...")

param_grid = {
    'alpha': [0.1, 0.5, 1.0, 1.5, 2.0],      # Параметр сглаживания
    'fit_prior': [True, False]                # Учитывать априорные вероятности
}
...
grid_nb = GridSearchCV(
    ...
    scoring='roc_auc',       # Используем ROC-AUC для оценки
...
print("Лучший ROC-AUC:", grid_nb.best_score_)
...
accuracy = accuracy_score(y_test, y_pred)
print(f"Accuracy: {accuracy:.4f}")
print("\nClassification Report для Naive Bayes GridSearch:")
print(classification_report(y_test, y_pred))
...
models = {
    "NaiveBayes": model,
    "GridNaiveBayes": grid_nb.best_estimator_
}
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC-кривые GridNaiveBayes и NaiveBayes')
plt.legend(loc="lower right")
